...
1. Verify both devices are on the correct network
   ...
   - In **Create Hotspot** mode, your computer/phone must be connected to the `InkPointX` hotspot
2. Double-check the IP address
   - Make sure you typed it correctly
   - Include `http://` at the beginning
   - Try the displayed IP address if `http://inkpoint.local/` does not resolve
