...
def generate_keys_header(
    ...
    # Position is load-bearing (it mirrors the old binary's index), so a
    # language removed from the active set cannot simply be dropped from this
    # list -- that would shift every code after it. Map it to EN instead,
    # same fallback a removed language's string code already gets elsewhere.
    v1_codes = [
        "EN", "ES", "FR", "DE", "CS", "PT", "RU", "SV", "RO", "CA", "UK",
        "BE", "IT", "PL", "FI", "DA", "NL", "TR", "KK", "HU", "LT", "SI",
    ]
    v1_enum_refs = [f"Language::{c}" if c in languages else "Language::EN" for c in v1_codes]
    lines.append("// V1 language.bin migration table (frozen enum order from 2f969a9)")
    lines.append("constexpr Language V1_LANGUAGES[] = {")
    lines.append("    " + ", ".join(v1_enum_refs) + ",")
    lines.append("};")
    lines.append(
