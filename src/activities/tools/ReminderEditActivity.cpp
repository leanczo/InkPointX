...
void ReminderEditActivity::handleSelection() {
  ...
  if (selectedIndex == 0) {
    ...
    startActivityForResult(
        makeUniqueNoThrow<ReminderDatePickerActivity>(renderer, mappedInput, editReminder.year, editReminder.month,
    ...
    startActivityForResult(
        makeUniqueNoThrow<KeyboardEntryActivity>(renderer, mappedInput, tr(STR_REMINDER_DESCRIPTION),
    ...
    startActivityForResult(makeUniqueNoThrow<ConfirmationActivity>(renderer, mappedInput, tr(STR_DELETE), body),
                           handler);
  }
}
