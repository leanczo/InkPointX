...
std::string promoteMarkdownHeaders(const std::string& raw) {
  ...
  while (lineStart <= raw.length()) {
    ...
    if (isHeader) {
      result += "\n\n";
      result += HtmlTextCleanup::kHeadingSentinel;
  ...
  static void XMLCALL endElement(void* userData, const XML_Char* name) {
    ...
    if (localTag == "item" || localTag == "entry") {
      ...
      if (!self->currentItem.title.empty() && self->itemsParsed < 25) {
        self->currentItem.title = HtmlTextCleanup::cleanField(self->currentItem.title);
        self->currentItem.link = HtmlTextCleanup::cleanField(self->currentItem.link);
        self->currentItem.description =
            HtmlTextCleanup::cleanField(promoteMarkdownHeaders(self->currentItem.description));
        stripMarkdownArtifacts(self->currentItem.description);
        self->currentItem.content = HtmlTextCleanup::cleanField(promoteMarkdownHeaders(self->currentItem.content));
      ...
      if (localTag == "title") {
        ...
        if (self->currentItem.timestamp.empty()) {
          uint32_t ts = parseRssDateToUnix(HtmlTextCleanup::cleanField(self->currentText));
...
void RssActivity::render(RenderLock&&) {
  ...
  if (state == RssState::Loading) {
    ...
    if (!errorMessage.empty() && allItems.empty()) {
      ...
      auto cardHeightFor = [&](const RssItem& item) {
        ...
        if (!item.description.empty()) {
          auto descLines = HtmlTextCleanup::wrapParagraphs(renderer, UI_10_FONT_ID, item.description, cellW - 24, kMaxDescLines,
    ...
    auto bodyLines =
        HtmlTextCleanup::wrapParagraphs(renderer, articleFontId, fullText, wrapWidth, 500, EpdFontFamily::REGULAR);
    ...
    // body together.
    std::vector<HtmlTextCleanup::DetailLine> lines;
