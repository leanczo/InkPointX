...
void RemindersListActivity::onEnter() {
  ...
  // Fresh boot with no clock sync yet has no valid time, which would show
  // every reminder as ~55 years overdue. Only fix this silently (no connect
  // prompt - this app should still work fully offline) when WiFi is already
  // associated; otherwise just flag it.
...
void RemindersListActivity::handleSelection() {
  ...
  auto handler = [this](const ActivityResult&) {
    ...
    // Resets selection to the top - the sort order may have changed, so
    // there's no stable position to restore.
    REMINDERS_STORE.loadFromFile();
    rebuildSortOrder();
    selectedIndex = 0;
    requestUpdate();
  ...
  startActivityForResult(makeUniqueNoThrow<ReminderEditActivity>(renderer, mappedInput, storeIndex, today), handler);
}
...
void RemindersListActivity::render(RenderLock&&) {
  ...
  GUI.drawScriptHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, tr(STR_REMINDERS_TITLE));
