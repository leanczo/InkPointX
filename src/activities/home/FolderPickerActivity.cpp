...
void FolderPickerActivity::render(RenderLock&&) {
  ...
  const auto pathText =
      renderer.truncatedText(SMALL_FONT_ID, basepath.c_str(), pageWidth - metrics.contentSidePadding * 2);
  const int pathX =
      I18N.isRtl() ? pageWidth - metrics.contentSidePadding - renderer.getTextWidth(SMALL_FONT_ID, pathText.c_str())
