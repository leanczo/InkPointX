...
void DiceActivity::drawD6(int x, int y, int size, int value) {
  ...
  renderer.drawRoundedRect(x - half, y - half, size, size, 2, 12, true);

  const auto pip = [&](int dx, int dy) { fillCircle(renderer, x + dx, y + dy, pipRadius, true); };
...
void DiceActivity::drawArrow(int x, int y, int size, int angle) {
  ...
  int endX = x + static_cast<int>(cosf(rad) * radius);
  int endY = y + static_cast<int>(sinf(rad) * radius);
  ...
  // Arrow head
  float headLen = 20.0f;
  float arrowAngle1 = rad + kPi * 0.85f;
  float arrowAngle2 = rad - kPi * 0.85f;

  int h1X = endX + static_cast<int>(cosf(arrowAngle1) * headLen);
  int h1Y = endY + static_cast<int>(sinf(arrowAngle1) * headLen);
  int h2X = endX + static_cast<int>(cosf(arrowAngle2) * headLen);
  int h2Y = endY + static_cast<int>(sinf(arrowAngle2) * headLen);
...
void DiceActivity::drawD20(int x, int y, int size, int value) {
  ...
  // Draw 20-sided icosahedron outline (Hexagon shape)
  int px[6];
  int py[6];
  for (int i = 0; i < 6; i++) {
    float angle = (i * 60.0f - 90.0f) * kPi / 180.0f;
    px[i] = x + static_cast<int>(cosf(angle) * radius);
    py[i] = y + static_cast<int>(sinf(angle) * radius);
  }
...
void DiceActivity::drawMagic8(int x, int y, int size, int responseIndex) {
  ...
  // Draw inner white circle for text
...
void DiceActivity::render(RenderLock&&) {
  ...
  GUI.drawScriptHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, tr(STR_DICE_TITLE));
