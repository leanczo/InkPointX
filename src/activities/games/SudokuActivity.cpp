...
void SudokuActivity::render(RenderLock&&) {
  ...
  int ngTextW = renderer.getTextWidth(SMALL_FONT_ID, tr(STR_SUDOKU_NEW_GAME));
  int ngTextX = toolbarBtnX1 + (toolbarBtnW - ngTextW) / 2;
  int ngTextY = toolbarY + (toolbarBtnH - renderer.getLineHeight(SMALL_FONT_ID)) / 2;
  renderer.drawText(SMALL_FONT_ID, ngTextX, ngTextY, tr(STR_SUDOKU_NEW_GAME), !newGameSelected);
  ...
  int checkTextW = renderer.getTextWidth(SMALL_FONT_ID, tr(STR_SUDOKU_CHECK));
  ...
  renderer.drawText(SMALL_FONT_ID, checkTextX, checkTextY, tr(STR_SUDOKU_CHECK), !checkSelected);

  // Grid: sized from actual available space (orientation-aware) instead of a
  // fixed box, so it fills as much of the screen as the theme's chrome
  ...
  if (hasWon) {
    ...
    // grid, so it can never run off the bottom of the screen once the grid
    // is tall.
