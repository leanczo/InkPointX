               ...
               size_t dstsize)   // I - Size of destination
{
  ...
  if (srclen <= dstsize) {
    ...
    if (ptr > dst && *ptr & 0x80) {
      while ((*ptr & 0xc0) == 0x80 && ptr > dst) ptr--;
              ...
              const char* s)      // I - String
{
  ...
  if (!pdf->loc) return (strtod(s, NULL));
...
void _pdfio_utf16cpy(char* dst,                 // I - Destination buffer for UTF-8
                     ...
                     size_t dstsize)            // I - Destination buffer size
{
  ...
  for (src += 2, srclen -= 2; srclen > 1 && dstptr < dstend; src += 2, srclen -= 2) {
    ...
    if (ch >= 0xd800 && ch <= 0xdbff && srclen > 3) {
      ...
      if (lch < 0xdc00 || lch >= 0xdfff) break;
  ...
  while (*format) {
    if (*format == '%') {
      ...
      *tptr++ = *format++;
      ...
      if (*format == '%') {
        if (bufptr < bufend) *bufptr++ = *format;
        bytes++;
        format++;
        continue;
      ...
      switch (type) {
        ...
        case 'g':
          ...
          if ((decptr = strstr(temp, dec)) != NULL) {
            ...
            if (tempptr > (decptr + 1)) memmove(decptr + 1, tempptr, strlen(tempptr) + 1);
            *decptr = '.';

            // Strip trailing 0's...
            for (tempptr = temp + strlen(temp) - 1; tempptr > temp && *tempptr == '0'; tempptr--) *tempptr = '\0';

            if (*tempptr == '.') *tempptr = '\0';  // Strip trailing decimal point
        ...
        case 's':  // Literal string
          ...
          if (width != 0) {
            ...
            if ((size_t)(width + 1) > sizeof(temp)) break;
...
_pdfioStringAllocBuffer(pdfio_file_t* pdf)  // I - PDF file
{
  ...
  if ((current = calloc(1, sizeof(_pdfio_strbuf_t))) == NULL) return (NULL);
...
pdfioStringCreate(pdfio_file_t* pdf,  // I - PDF file
                  const char* s)      // I - Nul-terminated string
{
  ...
  if ((news = strdup(s)) == NULL) return (NULL);
...
char*                                   // O - Durable string pointer or `NULL` on error
pdfioStringCreatef(pdfio_file_t* pdf,   // I - PDF file
                   const char* format,  // I - `printf`-style format string
...
void _pdfioStringFreeBuffer(pdfio_file_t* pdf,  // I - PDF file
...
_pdfioStringIsAllocated(pdfio_file_t* pdf,  // I - PDF file
                        const char* s)      // I - String
{
  int diff;  // Difference

  if (pdf->num_strings == 0) return (false);
