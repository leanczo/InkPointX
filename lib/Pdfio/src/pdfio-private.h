...
#define PDFIO_BUF_SIZE 1024    // File buffer size (ESP32-C3 tuned)
#define PDFIO_MAX_DEPTH 32     // Maximum nesting depth for values
#define PDFIO_MAX_STRING 8192  // Maximum object string length (ESP32-C3 tuned)
...
struct _pdfio_file_s  // PDF file structure
{
  ...
  pdfio_input_read_cb_t input_read_cb;
  // Optional callback-backed input
  pdfio_input_seek_cb_t input_seek_cb;
  // Optional callback-backed seek
  pdfio_input_close_cb_t input_close_cb;
  // Optional callback-backed close
  pdfio_xref_cb_t xref_cb;      // Optional disk-backed cross-reference table
  void* input_ctx;              // Callback-backed input context
...
struct _pdfio_obj_s  // Object
{
  ...
  size_t object_stream;       // Containing object stream for lazy compressed objects
...
struct _pdfio_stream_s  // Stream
{
  ...
  void *flate_state_storage,       // PNGdec zlib disables its own state allocation
      *flate_window_storage;       // Separate block avoids a fragile 40 KiB contiguous allocation
  bool flate_uses_shared;          // Storage belongs to the prepared document workspace
  bool cbuffer_uses_shared;        // Compressed input belongs to that workspace too
...
extern ssize_t _pdfio_vsnprintf(pdfio_file_t* pdf, char* buffer, size_t bufsize, const char* format,
                                va_list ap) _PDFIO_INTERNAL;
...
extern pdfio_array_t* _pdfioArrayRead(pdfio_file_t* pdf, pdfio_obj_t* obj, _pdfio_token_t* ts,
                                      size_t depth) _PDFIO_INTERNAL;
extern bool _pdfioArrayWrite(pdfio_array_t* a, pdfio_obj_t* obj) _PDFIO_INTERNAL;

extern void _pdfioCryptoAESInit(_pdfio_aes_t* ctx, const uint8_t* key, size_t keylen,
                                const uint8_t* iv) _PDFIO_INTERNAL;
extern size_t _pdfioCryptoAESDecrypt(_pdfio_aes_t* ctx, uint8_t* outbuffer, const uint8_t* inbuffer, size_t len,
                                     bool last) _PDFIO_INTERNAL;
extern size_t _pdfioCryptoAESEncrypt(_pdfio_aes_t* ctx, uint8_t* outbuffer, const uint8_t* inbuffer, size_t len,
                                     bool last) _PDFIO_INTERNAL;
extern bool _pdfioCryptoLock(pdfio_file_t* pdf, pdfio_permission_t permissions, pdfio_encryption_t encryption,
                             const char* owner_password, const char* user_password) _PDFIO_INTERNAL;
extern void _pdfioCryptoMakeRandom(uint8_t* buffer, size_t bytes) _PDFIO_INTERNAL;
extern _pdfio_crypto_cb_t _pdfioCryptoMakeReader(pdfio_file_t* pdf, pdfio_obj_t* obj, _pdfio_crypto_ctx_t* ctx,
                                                 uint8_t* iv, size_t* ivlen) _PDFIO_INTERNAL;
extern _pdfio_crypto_cb_t _pdfioCryptoMakeWriter(pdfio_file_t* pdf, pdfio_obj_t* obj, _pdfio_crypto_ctx_t* ctx,
                                                 uint8_t* iv, size_t* ivlen) _PDFIO_INTERNAL;
...
extern size_t _pdfioCryptoRC4Crypt(_pdfio_rc4_t* ctx, uint8_t* outbuffer, const uint8_t* inbuffer, size_t len,
                                   bool last) _PDFIO_INTERNAL;
...
extern pdfio_dict_t* _pdfioDictRead(pdfio_file_t* pdf, pdfio_obj_t* obj, _pdfio_token_t* ts,
                                    size_t depth) _PDFIO_INTERNAL;
...
extern pdfio_obj_t* _pdfioFileFindMappedObj(pdfio_file_t* pdf, pdfio_file_t* src_pdf,
                                            size_t src_number) _PDFIO_INTERNAL;
...
extern bool _pdfioFileLoadCompressedObj(pdfio_obj_t* obj) _PDFIO_INTERNAL;
...
extern pdfio_stream_t* _pdfioStreamCreate(pdfio_obj_t* obj, pdfio_obj_t* length_obj, size_t cbsize,
                                          pdfio_filter_t compression) _PDFIO_INTERNAL;
...
extern void _pdfioTokenInit(_pdfio_token_t* tb, pdfio_file_t* pdf, _pdfio_tconsume_cb_t consume_cb,
                            _pdfio_tpeek_cb_t peek_cb, void* cb_data);
...
extern _pdfio_value_t* _pdfioValueCopy(pdfio_file_t* pdfdst, _pdfio_value_t* vdst, pdfio_file_t* pdfsrc,
                                       _pdfio_value_t* vsrc) _PDFIO_INTERNAL;
...
extern _pdfio_value_t* _pdfioValueRead(pdfio_file_t* pdf, pdfio_obj_t* obj, _pdfio_token_t* ts, _pdfio_value_t* v,
                                       size_t depth) _PDFIO_INTERNAL;
