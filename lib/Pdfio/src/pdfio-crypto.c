...
static void decrypt_ou_key(pdfio_encryption_t encryption, const uint8_t* file_key, size_t file_keylen,
                           uint8_t ou_key[32]);
static void encrypt_ou_key(pdfio_encryption_t encryption, const uint8_t* file_key, size_t file_keylen,
                           uint8_t ou_key[32]);
static void make_file_key(pdfio_encryption_t encryption, size_t file_keylen, pdfio_permission_t permissions,
                          const unsigned char* file_id, size_t file_idlen, const uint8_t* user_pad,
                          const uint8_t* owner_key, bool encrypt_metadata, uint8_t file_key[16]);
static void make_owner_key(pdfio_encryption_t encryption, size_t file_keylen, const uint8_t* owner_pad,
                           const uint8_t* user_pad, uint8_t owner_key[32]);
...
static bool test_password(pdfio_encryption_t encryption, size_t file_keylen, pdfio_permission_t permisions,
                          const unsigned char* file_id, size_t file_idlen, const uint8_t* user_pad,
                          const uint8_t* user_key, const uint8_t* owner_key, bool encrypt_metadata,
                          uint8_t file_key[16]);
...
_pdfioCryptoLock(pdfio_file_t* pdf,               // I - PDF file
                 ...
                 const char* user_password)       // I - User password, if any
{
  ...
  switch (encryption) {
    ...
    case PDFIO_ENCRYPTION_AES_128:
      ...
      make_file_key(encryption, pdf->file_keylen, permissions, file_id, file_idlen, user_pad, pdf->owner_key,
                    pdf->encrypt_metadata, pdf->file_key);
...
void _pdfioCryptoMakeRandom(uint8_t* buffer,  // I - Buffer
                            size_t bytes)     // I - Number of bytes
{
  ...
  for (i = 1; i < 624; i++) mt_state[i] = (uint32_t)((1812433253 * (mt_state[i - 1] ^ (mt_state[i - 1] >> 30))) + i);
  ...
  while (bytes > 0) {
    if (mt_index == 0) {
      ...
      for (i = 0; i < 624; i++) {
        ...
        if (temp & 1) mt_state[i] ^= 2567483615u;
...
_pdfioCryptoMakeReader(pdfio_file_t* pdf,         // I  - PDF file
                       ...
                       size_t* ivlen)             // IO - Size of initialization vector
{
  ...
  PDFIO_DEBUG("_pdfioCryptoMakeReader(pdf=%p, obj=%p(%d), ctx=%p, iv=%p, ivlen=%p(%d))\n", (void*)pdf, (void*)obj,
              (int)obj->number, (void*)ctx, (void*)iv, (void*)ivlen, (int)*ivlen);
  ...
  switch (pdf->encryption) {
    ...
    case PDFIO_ENCRYPTION_RC4_128:
      ...
      if (pdf->encryption == PDFIO_ENCRYPTION_AES_128) _pdfioCryptoMD5Append(&md5, (const uint8_t*)"sAlT", 4);
      _pdfioCryptoMD5Finish(&md5, digest);
...
_pdfioCryptoMakeWriter(pdfio_file_t* pdf,         // I  - PDF file
                       pdfio_obj_t* obj,          // I  - PDF object
                       _pdfio_crypto_ctx_t* ctx,  // I  - Pointer to crypto context
                       uint8_t* iv,               // I  - Buffer for initialization vector
                       size_t* ivlen)             // IO - Size of initialization vector
...
bool                                                 // O - `true` on success, `false` otherwise
_pdfioCryptoUnlock(pdfio_file_t* pdf,                // I - PDF file
                   ...
                   void* password_data)              // I - Password callback data, if any
{
  ...
  if ((value = _pdfioDictGetValue(pdf->encrypt_dict, "EncryptMetadata")) != NULL &&
      value->type == PDFIO_VALTYPE_BOOLEAN)
  ...
  PDFIO_DEBUG("_pdfioCryptoUnlock: handler=%p(%s), version=%d, revision=%d, length=%d, encrypt_metadata=%s\n",
              ...
              pdf->encrypt_metadata ? "true" : "false");
  ...
  if (version == 4 && revision == 4) {
    ...
    if (!cf_dict) {
      ...
      if (length < 40 || length > 128) length = 128;  // Default to 128 bits
    ...
    if (revision == 2) {
      ...
      pdf->encryption = PDFIO_ENCRYPTION_RC4_128;
      if (length < 40 || length > 128) length = 128;
    }
  ...
  PDFIO_DEBUG("_pdfioCryptoUnlock: owner_key[%d]=%02X%02X%02X%02X...%02X%02X%02X%02X\n", (int)owner_keylen,
              owner_key[0], owner_key[1], owner_key[2], owner_key[3], owner_key[28], owner_key[29], owner_key[30],
              owner_key[31]);
  ...
  PDFIO_DEBUG("_pdfioCryptoUnlock: user_key[%d]=<%02X%02X%02X%02X...%02X%02X%02X%02X>\n", (int)user_keylen, user_key[0],
              user_key[1], user_key[2], user_key[3], user_key[28], user_key[29], user_key[30], user_key[31]);
  ...
  PDFIO_DEBUG("_pdfioCryptoUnlock: file_id(%d)=<%02X%02X%02X%02X...%02X%02X%02X%02X>\n", (int)file_idlen, file_id[0],
              file_id[1], file_id[2], file_id[3], file_id[12], file_id[13], file_id[14], file_id[15]);
  ...
  for (tries = 0; tries < 4; tries++) {
    if (pdf->encryption <= PDFIO_ENCRYPTION_AES_128) {
      ...
      PDFIO_DEBUG("_pdfioCryptoUnlock: Trying <%02X%02X%02X%02X...%02X%02X%02X%02X>\n", pad[0], pad[1], pad[2], pad[3],
                  pad[28], pad[29], pad[30], pad[31]);

      if (test_password(pdf->encryption, pdf->file_keylen, pdf->permissions, file_id, file_idlen, pad, pdf->user_key,
                        pdf->owner_key, pdf->encrypt_metadata, file_key)) {
    ...
    if (password_cb) password = (password_cb)(password_data, pdf->filename);
...
static void decrypt_ou_key(pdfio_encryption_t encryption,  // I  - Type of encryption
                           ...
                           uint8_t ou_key[32])             // IO - Owner/User key
{
  ...
  if (encryption == PDFIO_ENCRYPTION_RC4_40) {
    ...
    for (i = 20; i > 0;) {
      ...
      for (j = 0; j < file_keylen; j++) key[j] = (uint8_t)(file_key[j] ^ i);
...
static void encrypt_ou_key(pdfio_encryption_t encryption,  // I  - Type of encryption
                           const uint8_t* file_key,        // I  - File encryption key
                           size_t file_keylen,             // I  - Length of file encryption key
                           uint8_t ou_key[32])             // IO - Owner/User key
{
...
static void make_file_key(pdfio_encryption_t encryption,   // I - Type of encryption
                          ...
                          uint8_t file_key[16])  // O - Encryption key
{
  ...
  PDFIO_DEBUG("make_file_key: user_pad[32]=<%02X%02X%02X%02X...%02X%02X%02X%02X>\n", user_pad[0], user_pad[1],
              user_pad[2], user_pad[3], user_pad[28], user_pad[29], user_pad[30], user_pad[31]);
  PDFIO_DEBUG("make_file_key: owner_key[32]=<%02X%02X%02X%02X...%02X%02X%02X%02X>\n", owner_key[0], owner_key[1],
              owner_key[2], owner_key[3], owner_key[28], owner_key[29], owner_key[30], owner_key[31]);
  PDFIO_DEBUG("make_file_key: permissions(%d)=<%02X%02X%02X%02X>\n", permissions, perm_bytes[0], perm_bytes[1],
              perm_bytes[2], perm_bytes[3]);
...
static void make_owner_key(pdfio_encryption_t encryption,  // I - Type of encryption
...
static void make_password_key(pdfio_encryption_t encryption,  // I - Type of encryption
                              size_t file_keylen,             // I - Length of encryption key
...
static void make_user_key(const unsigned char* file_id,  // I - File ID value
                          size_t file_idlen,             // I - Length of file ID
...
static void pad_password(const char* password,  // I - Password string or `NULL`
                         uint8_t pad[32])       // O - Padded password
{
  ...
  if (password) {
    ...
    if ((len = strlen(password)) > 32) len = 32;
  } else {
    // No password
    len = 0;
  }

  if (len > 0) memcpy(pad, password, len);
  if (len < 32) memcpy(pad + len, pdf_passpad, 32 - len);
}
...
test_password(pdfio_encryption_t encryption,   // I - Type of encryption
              size_t file_keylen,              // I - Length of encryption key
              pdfio_permission_t permissions,  // I - File permissions
              const unsigned char* file_id,    // I - File ID value
              size_t file_idlen,               // I - Length of file ID
              ...
              uint8_t file_key[16])  // O - Encryption key
{
  ...
  // Make the file key...
  make_file_key(encryption, file_keylen, permissions, file_id, file_idlen, user_pad, owner_key, encrypt_metadata,
                file_key);
  ...
  PDFIO_DEBUG("test_password: pdf_user_key[32]=<%02X%02X%02X%02X...%02X%02X%02X%02X>\n", pdf_user_key[0],
              pdf_user_key[1], pdf_user_key[2], pdf_user_key[3], pdf_user_key[28], pdf_user_key[29], pdf_user_key[30],
              pdf_user_key[31]);
  ...
  if (encryption == PDFIO_ENCRYPTION_RC4_40 && !memcmp(pdf_user_key, pdf_passpad, 32)) {
    ...
    PDFIO_DEBUG("test_password: own_user_key[32]=<%02X%02X%02X%02X...%02X%02X%02X%02X>\n", own_user_key[0],
                own_user_key[1], own_user_key[2], own_user_key[3], own_user_key[28], own_user_key[29], own_user_key[30],
                own_user_key[31]);
