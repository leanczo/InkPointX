...
// Error callback
typedef ssize_t (*pdfio_input_read_cb_t)(void* ctx, void* data, size_t datalen);
// Input callback for callback-backed files
typedef off_t (*pdfio_input_seek_cb_t)(void* ctx, off_t offset, int whence);
// Seek callback for callback-backed files
typedef void (*pdfio_input_close_cb_t)(void* ctx);
// Close callback for callback-backed files
typedef struct pdfio_xref_s {
  off_t offset;               // Direct object file offset
  size_t object_stream;       // Containing object stream, if compressed
  unsigned short generation;  // Object generation
  unsigned char type;         // 0=missing/free, 1=direct, 2=compressed
} pdfio_xref_t;
typedef bool (*pdfio_xref_cb_t)(void* ctx, size_t number, pdfio_xref_t* record, bool write_record);
// Disk-backed cross-reference callback
...
enum pdfio_permission_e  // PDF permission bits
{
  ...
  PDFIO_PERMISSION_ASSEMBLE =
      0x0400,  // PDF allows assembly (insert, delete, or rotate pages, add document outlines and thumbnails)
...
extern bool pdfioDictSetBinary(pdfio_dict_t* dict, const char* key, const unsigned char* value,
                               size_t valuelen) _PDFIO_PUBLIC;
...
extern pdfio_file_t* pdfioFileCreate(const char* filename, const char* version, pdfio_rect_t* media_box,
                                     pdfio_rect_t* crop_box, pdfio_error_cb_t error_cb, void* error_data) _PDFIO_PUBLIC;
...
extern pdfio_file_t* pdfioFileCreateOutput(pdfio_output_cb_t output_cb, void* output_ctx, const char* version,
                                           pdfio_rect_t* media_box, pdfio_rect_t* crop_box, pdfio_error_cb_t error_cb,
                                           void* error_data) _PDFIO_PUBLIC;
...
extern pdfio_file_t* pdfioFileCreateTemporary(char* buffer, size_t bufsize, const char* version,
                                              pdfio_rect_t* media_box, pdfio_rect_t* crop_box,
                                              pdfio_error_cb_t error_cb, void* error_data) _PDFIO_PUBLIC;
...
extern pdfio_file_t* pdfioFileOpen(const char* filename, pdfio_password_cb_t password_cb, void* password_data,
                                   pdfio_error_cb_t error_cb, void* error_data) _PDFIO_PUBLIC;
extern pdfio_file_t* pdfioFileOpenIO(const char* name, pdfio_input_read_cb_t read_cb, pdfio_input_seek_cb_t seek_cb,
                                     pdfio_input_close_cb_t close_cb, pdfio_xref_cb_t xref_cb, void* input_ctx,
                                     pdfio_password_cb_t password_cb, void* password_data, pdfio_error_cb_t error_cb,
                                     void* error_data) _PDFIO_PUBLIC;
...
extern bool pdfioFileSetPermissions(pdfio_file_t* pdf, pdfio_permission_t permissions, pdfio_encryption_t encryption,
                                    const char* owner_password, const char* user_password) _PDFIO_PUBLIC;
...
extern bool pdfioPrepareFlateWorkspace(void) _PDFIO_PUBLIC;
extern void pdfioReleaseFlateWorkspace(void) _PDFIO_PUBLIC;
