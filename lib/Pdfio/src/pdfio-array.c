...
pdfioArrayAppendArray(pdfio_array_t* a,      // I - Array
...
bool                                                // O - `true` on success, `false` on failure
pdfioArrayAppendBinary(pdfio_array_t* a,            // I - Array
                       ...
                       size_t valuelen)             // I - Length of value
{
  ...
  // Range check input
  if (!a || !value || !valuelen) return (false);
...
bool                                       // O - `true` on success, `false` on failure
pdfioArrayAppendBoolean(pdfio_array_t* a,  // I - Array
                        bool value)        // I - Value
{
  _pdfio_value_t v;  // Value for array
...
bool                                    // O - `true` on success, `false` on failure
pdfioArrayAppendDate(pdfio_array_t* a,  // I - Array
...
bool                                       // O - `true` on success, `false` on failure
pdfioArrayAppendDict(pdfio_array_t* a,     // I - Array
...
bool                                     // O - `true` on success, `false` on failure
pdfioArrayAppendName(pdfio_array_t* a,   // I - Array
...
bool                                      // O - `true` on success, `false` on failure
pdfioArrayAppendNumber(pdfio_array_t* a,  // I - Array
...
bool                                     // O - `true` on success, `false` on failure
pdfioArrayAppendObj(pdfio_array_t* a,    // I - Array
                    pdfio_obj_t* value)  // I - Value
{
  _pdfio_value_t v;  // Value for array

  // Range check input
  if (!a || !value || a->pdf != value->pdf) return (false);
...
bool                                       // O - `true` on success, `false` on failure
pdfioArrayAppendString(pdfio_array_t* a,   // I - Array
                       const char* value)  // I - Value
{
  _pdfio_value_t v;  // Value for array
               ...
               pdfio_array_t* a)   // I - Original array
{
  ...
  // Range check input...
  if (!pdf || !a) return (NULL);

  // Create the new array...
  if ((na = pdfioArrayCreate(pdf)) == NULL) return (NULL);
  ...
  for (i = a->num_values, vsrc = a->values; i > 0; i--, vsrc++) {
    if (!_pdfioValueCopy(pdf, &vdst, a->pdf, vsrc)) return (NULL);  // Let pdfioFileClose do the cleanup...
...
pdfioArrayCreate(pdfio_file_t* pdf)  // I - PDF file
{
  ...
  if ((a = (pdfio_array_t*)calloc(1, sizeof(pdfio_array_t))) == NULL) return (NULL);
                   ...
                   size_t depth)       // I - Depth
{
  ...
  for (i = a->num_values, v = a->values; i > 0; i--, v++) {
    if (!_pdfioValueDecrypt(pdf, obj, v, depth)) return (false);
  }
...
void _pdfioArrayDebug(pdfio_array_t* a,  // I - Array
                      FILE* fp)          // I - Output file
{
  ...
  putc('[', fp);
  for (i = a->num_values, v = a->values; i > 0; i--, v++) _pdfioValueDebug(v, fp);
  putc(']', fp);
}
...
void _pdfioArrayDelete(pdfio_array_t* a)  // I - Array
{
  size_t i;  // Looping var

  for (i = 0; i < a->num_values; i++) {
    if (a->values[i].type == PDFIO_VALTYPE_BINARY) free(a->values[i].value.binary.data);
  }
...
pdfioArrayGetBinary(pdfio_array_t* a,  // I - Array
                    size_t n,          // I - Index
                    size_t* length)    // O - Length of string
{
  if (!a || n >= a->num_values ||
      (a->values[n].type != PDFIO_VALTYPE_BINARY && a->values[n].type != PDFIO_VALTYPE_STRING)) {
    if (length) *length = 0;

    return (NULL);
  } else if (a->values[n].type == PDFIO_VALTYPE_BINARY) {
    if (length) *length = a->values[n].value.binary.datalen;

    return (a->values[n].value.binary.data);
  } else {
    if (length) *length = strlen(a->values[n].value.string);
                ...
                size_t depth)        // I - Depth of array
{
  ...
  // Create an array...
  if ((array = pdfioArrayCreate(pdf)) == NULL) return (NULL);
  ...
  while (_pdfioTokenGet(tb, token, sizeof(token))) {
    ...
    if (!_pdfioValueRead(pdf, obj, tb, &value, depth)) break;
...
pdfioArrayRemove(pdfio_array_t* a,  // I - Array
                 size_t n)          // I - Index
{
  if (!a || n >= a->num_values) return (false);

  if (a->values[n].type == PDFIO_VALTYPE_BINARY) free(a->values[n].value.binary.data);

  a->num_values--;
  if (n < a->num_values) memmove(a->values + n, a->values + n + 1, (a->num_values - n) * sizeof(_pdfio_value_t));
                 ...
                 pdfio_obj_t* obj)  // I - Object, if any
{
  ...
  // Write each value...
  for (i = a->num_values, v = a->values; i > 0; i--, v++) {
    if (!_pdfioValueWrite(pdf, obj, v, NULL)) return (false);
  }
