               ...
               const char* key)     // I - Key
{
  ...
  if (dict->num_pairs > 0) {
    ...
    if ((pair = (_pdfio_pair_t*)bsearch(&pkey, dict->pairs, dict->num_pairs, sizeof(_pdfio_pair_t),
                                        (int (*)(const void*, const void*))compare_pairs)) != NULL) {
      // Yes, remove it...
      if (pair->value.type == PDFIO_VALTYPE_BINARY) free(pair->value.value.binary.data);

      idx = (size_t)(pair - dict->pairs);
      dict->num_pairs--;

      if (idx < dict->num_pairs) memmove(pair, pair + 1, (dict->num_pairs - idx) * sizeof(_pdfio_pair_t));
              ...
              pdfio_dict_t* dict)  // I - Original dictionary
{
  ...
  if ((ndict = pdfioDictCreate(pdf)) == NULL) return (NULL);
  ...
  for (i = dict->num_pairs, p = dict->pairs; i > 0; i--, p++) {
    ...
    if (!key) return (NULL);  // Let pdfioFileClose do the cleanup...
...
pdfioDictCreate(pdfio_file_t* pdf)  // I - PDF file
{
  ...
  if ((dict = (pdfio_dict_t*)calloc(1, sizeof(pdfio_dict_t))) == NULL) return (NULL);
                  ...
                  size_t depth)        // I - Depth
{
  ...
  for (i = dict->num_pairs, pair = dict->pairs; i > 0; i--, pair++) {
    if (strcmp(pair->key, "ID") && !_pdfioValueDecrypt(pdf, obj, &pair->value, depth + 1)) return (false);
  }
                   ...
                   const char* key)     // I - Key
{
  ...
  if (value && value->type == PDFIO_VALTYPE_STRING) {
    ...
    if (!(value->value.binary.datalen & 1) &&
        (!memcmp(value->value.binary.data, "\376\377", 2) || !memcmp(value->value.binary.data, "\377\376", 2))) {
      // Copy UTF-16...
                   ...
                   const char* key)     // I - Key
{
  ...
  if ((match = bsearch(&temp, dict->pairs, dict->num_pairs, sizeof(_pdfio_pair_t),
                       (int (*)(const void*, const void*))compare_pairs)) != NULL) {
...
void pdfioDictIterateKeys(pdfio_dict_t* dict,  // I - Dictionary
                          ...
                          void* cb_data)       // I - Callback data
{
  ...
  for (i = dict->num_pairs, pair = dict->pairs; i > 0; i--, pair++) {
    if (!(cb)(dict, pair->key, cb_data)) break;
  }
}
...
bool                                            // O - `true` on success, `false` on failure
pdfioDictSetBinary(pdfio_dict_t* dict,          // I - Dictionary
                   const char* key,             // I - Key
                   ...
                   size_t valuelen)             // I - Length of value
{
  ...
  // Range check input...
  if (!dict || !key || !value || !valuelen) return (false);

  // Set the key/value pair...
  temp.type = PDFIO_VALTYPE_BINARY;
  temp.value.binary.datalen = valuelen;

  if ((temp.value.binary.data = (unsigned char*)malloc(valuelen)) == NULL) return (false);
...
bool                                     // O - `true` on success, `false` on failure
pdfioDictSetStringf(pdfio_dict_t* dict,  // I - Dictionary
                    const char* key,     // I - Key
                    ...
                    ...)                 // I - Additional arguments as needed
{
  ...
  // Range check input...
  if (!dict || !key || !format) return (false);
...
bool                                       // O - `true` on success, `false` on failure
_pdfioDictSetValue(pdfio_dict_t* dict,     // I - Dictionary
                   const char* key,        // I - Key
                ...
                off_t* length)       // I - Offset to length value
{
  ...
  for (i = dict->num_pairs, pair = dict->pairs; i > 0; i--, pair++) {
    if (!_pdfioFilePrintf(pdf, "%N", pair->key)) return (false);

    if (length && !strcmp(pair->key, "Length") && pair->value.type == PDFIO_VALTYPE_NUMBER &&
        pair->value.value.number <= 0.0) {
      // Writing an object dictionary with an undefined length
      *length = _pdfioFileTell(pdf) + 1;
      if (!_pdfioFilePuts(pdf, " 9999999999")) return (false);
