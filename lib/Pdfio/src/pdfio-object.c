...
pdfioObjClose(pdfio_obj_t* obj)  // I - Object
{
  ...
  if (!obj->offset) {
    ...
    if (!_pdfioObjWriteHeader(obj)) return (false);
             ...
             pdfio_obj_t* srcobj)  // I - Object to copy
{
  ...
  PDFIO_DEBUG("pdfioObjCopy(pdf=%p, srcobj=%p(%u,%p))\n", (void*)pdf, (void*)srcobj,
              srcobj ? (unsigned)srcobj->number : 0, srcobj ? (void*)srcobj->pdf : NULL);

  // Range check input
  if (!pdf || !srcobj) return (NULL);

  // Load the object value if needed...
  if (srcobj->value.type == PDFIO_VALTYPE_NONE) {
    if (!_pdfioObjLoad(srcobj)) return (NULL);
  }
  ...
  if ((dstobj = _pdfioFileCreateObj(pdf, srcobj->pdf, NULL)) == NULL) return (NULL);

  // Add new object to the cache of copied objects...
  if (!_pdfioFileAddMappedObj(pdf, dstobj, srcobj)) return (NULL);

  // Copy the object's value...
  if (!_pdfioValueCopy(pdf, &dstobj->value, srcobj->pdf, &srcobj->value)) return (NULL);

  if (dstobj->value.type == PDFIO_VALTYPE_DICT) pdfioDictClear(dstobj->value.value.dict, "Length");
...
pdfioObjCreateStream(pdfio_obj_t* obj,       // I - Object
                     pdfio_filter_t filter)  // I - Type of compression to apply
{
  ...
  // Range check input
  if (!obj || obj->pdf->mode != _PDFIO_MODE_WRITE || obj->value.type != PDFIO_VALTYPE_DICT) return (NULL);
  ...
  if (!_pdfioObjWriteHeader(obj)) return (NULL);
  ...
  if ((st = _pdfioStreamCreate(obj, length_obj, 0, filter)) != NULL) obj->pdf->current_obj = obj;
...
void _pdfioObjDelete(pdfio_obj_t* obj)  // I - Object
{
  if (obj) {
    pdfioStreamClose(obj->stream);

    if (obj->datafree) (obj->datafree)(obj->data);
  }
...
pdfioObjGetLength(pdfio_obj_t* obj)  // I - Object
{
  ...
  // Range check input...
  if (!obj || !obj->stream_offset || obj->value.type != PDFIO_VALTYPE_DICT) return (0);
  ...
  if (lenobj->value.type == PDFIO_VALTYPE_NONE) _pdfioObjLoad(lenobj);
...
_pdfioObjLoad(pdfio_obj_t* obj)  // I - Object
{
  ...
  PDFIO_DEBUG("_pdfioObjLoad(obj=%p(%lu)), offset=%lu\n", (void*)obj, (unsigned long)obj->number,
              (unsigned long)obj->offset);

  // Compressed objects do not have a direct file offset.  Callback-backed
  // readers load just the requested value from its object stream on demand.
  if (!obj->offset && obj->object_stream) return (_pdfioFileLoadCompressedObj(obj));
                   ...
                   bool decode)       // I - Decode/decompress data?
{
  ...
  if (!obj->value.type) {
    if (!_pdfioObjLoad(obj)) return (NULL);
  }
  ...
  // Open the stream...
  if ((st = _pdfioStreamOpen(obj, decode)) != NULL) obj->pdf->current_obj = obj;
...
void _pdfioObjSetExtension(pdfio_obj_t* obj,           // I - Object
                           void* data,                 // I - Data
...
_pdfioObjWriteHeader(pdfio_obj_t* obj)  // I - Object
{
  ...
  if (!_pdfioFilePrintf(obj->pdf, "%lu %u obj\n", (unsigned long)obj->number, obj->generation)) return (false);

  if (!_pdfioValueWrite(obj->pdf, obj, &obj->value, &obj->length_offset)) return (false);
