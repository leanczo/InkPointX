...
static pdfio_file_t* create_common(const char* filename, int fd, pdfio_output_cb_t output_cb, void* output_cbdata,
                                   const char* version, pdfio_rect_t* media_box, pdfio_rect_t* crop_box,
                                   pdfio_error_cb_t error_cb, void* error_cbdata);
static pdfio_obj_t* find_loaded_obj(pdfio_file_t* pdf, size_t number);
...
static pdfio_obj_t* get_lazy_page(pdfio_file_t* pdf, pdfio_obj_t* obj, size_t page, size_t depth);
...
static bool store_xref(pdfio_file_t* pdf, size_t number, const pdfio_xref_t* xref);
...
_pdfioFileAddMappedObj(pdfio_file_t* pdf,     // I - Destination PDF file
...
pdfioFileClose(pdfio_file_t* pdf)  // I - PDF file
{
...
#ifndef PDFIO_READ_ONLY
    // Add default OutputIntent for PDF/A CMYK printing...
    pdfioFileAddOutputIntent(pdf, /*subtype*/ "GTS_PDFA1", /*condition*/ "CMYK", /*cond_id*/ "CGATS001",
                             /*reg_name*/ NULL, /*info*/ "CMYK Printing", /*profile*/ NULL);
#endif  // !PDFIO_READ_ONLY

    // Close and write out the last bits...
    if (write_metadata(pdf) && pdfioObjClose(pdf->info_obj) && write_pages(pdf) && pdfioObjClose(pdf->root_obj) &&
        write_trailer(pdf))
      ret = _pdfioFileFlush(pdf);
  }

  if (pdf->input_close_cb)
    (pdf->input_close_cb)(pdf->input_ctx);
  else if (pdf->fd >= 0 && close(pdf->fd) < 0)
    ret = false;
  ...
  for (i = 0; i < pdf->num_arrays; i++) _pdfioArrayDelete(pdf->arrays[i]);
  free(pdf->arrays);

  for (i = 0; i < pdf->num_dicts; i++) _pdfioDictDelete(pdf->dicts[i]);
  free(pdf->dicts);

  for (i = 0; i < pdf->num_objs; i++) _pdfioObjDelete(pdf->objs[i]);
  free(pdf->objs);
  ...
  for (i = 0; i < pdf->num_strings; i++) free(pdf->strings[i]);
  free(pdf->strings);
...
pdfioFileCreate(const char* filename,       // I - Filename
                ...
                void* error_cbdata)         // I - Error callback data, if any
{
  ...
  PDFIO_DEBUG(
      "pdfioFileCreate(filename=\"%s\", version=\"%s\", media_box=%p, crop_box=%p, error_cb=%p, error_cbdata=%p)\n",
  ...
  if ((fd = open(filename, O_WRONLY | O_BINARY | O_CREAT | O_TRUNC, 0666)) < 0) {
    ...
    char message[512];  // Message string
  ...
  if ((pdf = create_common(filename, fd, /*output_cb*/ NULL, /*output_cbdata*/ NULL, version, media_box, crop_box,
                           error_cb, error_cbdata)) == NULL) {
...
pdfioFileCreateArrayObj(pdfio_file_t* pdf,     // I - PDF file
                        pdfio_array_t* array)  // I - Object array
{
  _pdfio_value_t value;  // Object value

  // Range check input...
  if (!pdf || !array) return (NULL);
...
pdfio_obj_t*                               // O - New object
pdfioFileCreateNameObj(pdfio_file_t* pdf,  // I - PDF file
...
pdfio_obj_t*                                 // O - New object
pdfioFileCreateNumberObj(pdfio_file_t* pdf,  // I - PDF file
...
pdfio_obj_t*                            // O - New object
pdfioFileCreateObj(pdfio_file_t* pdf,   // I - PDF file
                   pdfio_dict_t* dict)  // I - Object dictionary
{
  _pdfio_value_t value;  // Object value

  // Range check input...
  if (!pdf || !dict) return (NULL);
...
pdfio_obj_t*                                // O - New object
_pdfioFileCreateObj(pdfio_file_t* pdf,      // I - PDF file
                    ...
                    _pdfio_value_t* value)  // I - Object dictionary
{
  ...
  if (pdf->mode != _PDFIO_MODE_WRITE) return (NULL);
  ...
  if (value) _pdfioValueCopy(pdf, &obj->value, srcpdf, value);
...
pdfio_file_t*                                       // O - PDF file or `NULL` on error
pdfioFileCreateOutput(pdfio_output_cb_t output_cb,  // I - Output callback function
...
{
  PDFIO_DEBUG(
      "pdfioFileCreate(output_cb=%p, output_cbdata=%p, version=\"%s\", media_box=%p, crop_box=%p, error_cb=%p, "
      "error_cbdata=%p)\n",
      (void*)output_cb, (void*)output_cbdata, version, (void*)media_box, (void*)crop_box, (void*)error_cb,
      (void*)error_cbdata);

  return (create_common("output.pdf", /*fd*/ -1, output_cb, output_cbdata, version, media_box, crop_box, error_cb,
                        error_cbdata));
}
                    ...
                    pdfio_dict_t* dict)  // I - Page dictionary
{
  ...
  if (!_pdfioDictGetValue(dict, "CropBox")) pdfioDictSetRect(dict, "CropBox", &pdf->crop_box);

  if (!_pdfioDictGetValue(dict, "MediaBox")) pdfioDictSetRect(dict, "MediaBox", &pdf->media_box);
  ...
  if (!_pdfioDictGetValue(dict, "Type")) pdfioDictSetName(dict, "Type", "Page");

  // Create the page object...
  if ((page = pdfioFileCreateObj(pdf, dict)) == NULL) return (NULL);

  // Create a contents object to hold the contents of the page...
  if ((contents_dict = pdfioDictCreate(pdf)) == NULL) return (NULL);

#ifndef DEBUG
  pdfioDictSetName(contents_dict, "Filter", "FlateDecode");
#endif  // !DEBUG

  if ((contents = pdfioFileCreateObj(pdf, contents_dict)) == NULL) return (NULL);

  // Add the contents stream to the pages object and write it...
  pdfioDictSetObj(dict, "Contents", contents);
  if (!pdfioObjClose(page)) return (NULL);

  if (!_pdfioFileAddPage(pdf, page)) return (NULL);
...
pdfio_obj_t*                                  // O - New object
pdfioFileCreateStringObj(pdfio_file_t* pdf,   // I - PDF file
...
pdfio_file_t* pdfioFileCreateTemporary(char* buffer,               // I - Filename buffer
                                       ...
                                       void* error_cbdata)         // I - Error callback data, if any
{
  ...
  PDFIO_DEBUG(
      "pdfioFileCreate(buffer=%p, bufsize=%lu, version=\"%s\", media_box=%p, crop_box=%p, error_cb=%p, "
      "error_cbdata=%p)\n",
      (void*)buffer, (unsigned long)bufsize, version, (void*)media_box, (void*)crop_box, (void*)error_cb,
      (void*)error_cbdata);
  ...
  if ((tmpdir = getenv("TMPDIR")) != NULL && access(tmpdir, W_OK)) tmpdir = NULL;
...
#else
  if ((tmpdir = getenv("TMPDIR")) == NULL || access(tmpdir, W_OK)) tmpdir = "/tmp";
#endif  // _WIN32

  for (i = 0, fd = -1; i < 1000; i++) {
    _pdfioCryptoMakeRandom((uint8_t*)&tmpnum, sizeof(tmpnum));
    snprintf(buffer, bufsize, "%s/%08x.pdf", tmpdir, tmpnum);
    if ((fd = open(buffer, O_WRONLY | O_BINARY | O_CREAT | O_TRUNC | O_EXCL, 0666)) >= 0) break;
  }
  ...
  if ((pdf = create_common(buffer, fd, /*output_cb*/ NULL, /*output_cbdata*/ NULL, version, media_box, crop_box,
                           error_cb, error_cbdata)) == NULL) {
...
_pdfioFileFindMappedObj(pdfio_file_t* pdf,      // I - Destination PDF file
                        ...
                        size_t src_number)      // I - Source object number
{
  ...
  if (pdf->num_objmaps == 0) return (NULL);

  // Otherwise search for a match...
  memcpy(key.src_id, src_pdf->file_id, sizeof(key.src_id));
  key.src_number = src_number;

  if ((match = (_pdfio_objmap_t*)bsearch(&key, pdf->objmaps, pdf->num_objmaps, sizeof(_pdfio_objmap_t),
                                         (int (*)(const void*, const void*))compare_objmaps)) != NULL)
    return (match->obj);
  else
    return (NULL);
}
...
pdfioFileFindObj(pdfio_file_t* pdf,  // I - PDF file
                 size_t number)      // I - Object number (1 to N)
{
  pdfio_obj_t* obj;   // Loaded object
  pdfio_xref_t xref;  // Lazy cross-reference record

  // First look in the small set of objects that are already resident.
  if (!pdf || number < 1) return (NULL);

  if ((obj = find_loaded_obj(pdf, number)) != NULL) return (obj);

  // Callback-backed readers keep the full xref on storage and instantiate
  // objects only when a dictionary actually references them.
  if (!pdf->xref_cb) return (NULL);

  memset(&xref, 0, sizeof(xref));
  if (!(pdf->xref_cb)(pdf->input_ctx, number, &xref, false) || !xref.type) return (NULL);

  if (xref.type == 1)
    return (add_obj(pdf, number, xref.generation, xref.offset));
  else if (xref.type == 2) {
    if ((obj = add_obj(pdf, number, 0, 0)) != NULL) obj->object_stream = xref.object_stream;
    return (obj);
  }
...
//
// 'find_loaded_obj()' - Find an object already resident in memory.
//
...
static pdfio_obj_t* find_loaded_obj(pdfio_file_t* pdf, size_t number) {
  ...
  PDFIO_DEBUG("pdfioFileFindObj(pdf=%p, number=%lu) alloc_objs=%lu, num_objs=%lu, objs=%p\n", (void*)pdf,
              (unsigned long)number, (unsigned long)(pdf ? pdf->alloc_objs : 0),
              (unsigned long)(pdf ? pdf->num_objs : 0), (void*)(pdf ? pdf->objs : NULL));

  // Range check input...
  if (!pdf || pdf->num_objs == 0 || number < 1) return (NULL);

  // Do a binary search for the object...
  if ((current = number - 1) >= pdf->num_objs) current = pdf->num_objs / 2;

  PDFIO_DEBUG("pdfioFileFindObj: objs[current=%lu]=%p(%lu)\n", (unsigned long)current, (void*)pdf->objs[current],
              (unsigned long)(pdf->objs[current] ? pdf->objs[current]->number : 0));
...
pdfioFileGetCreationDate(pdfio_file_t* pdf)  // I - PDF file
{
...
pdfioFileGetModificationDate(pdfio_file_t* pdf)  // I - PDF file
{
...
pdfioFileGetNumObjs(pdfio_file_t* pdf)  // I - PDF file
{
                 ...
                 size_t n)           // I - Page index (starting at 0)
{
  if (!pdf || n >= pdf->num_pages)
    return (NULL);
  else if (pdf->xref_cb && pdf->pages_obj)
    return (get_lazy_page(pdf, pdf->pages_obj, n, 0));
  else
    return (pdf->pages[n]);
}
...
pdfioFileGetPermissions(pdfio_file_t* pdf,               // I - PDF file
                        pdfio_encryption_t* encryption)  // O - Type of encryption used or `NULL` to ignore
{
  // Range check input...
  if (!pdf) {
    if (encryption) *encryption = PDFIO_ENCRYPTION_NONE;

    return (PDFIO_PERMISSION_ALL);
  }

  // Return values...
  if (encryption) *encryption = pdf->encryption;
...
pdfioFileGetVersion(pdfio_file_t* pdf)  // I - PDF file
{
...
pdfioFileOpen(const char* filename,             // I - Filename
              ...
              void* error_cbdata)         // I - Error callback data, if any
{
  ...
  if (!_pdfioFileGets(pdf, line, sizeof(line), true)) goto error;
  ...
  for (ptr = line; ptr < end; ptr++) {
    if (!strncmp(ptr, "startxref", 9) && !strstr(ptr + 9, "startxref") && strtol(ptr + 9, NULL, 10) > 0) break;
  }
  ...
  if (ptr >= end) {
    ...
    if (!repair_xref(pdf, password_cb, password_cbdata)) goto error;
  } else {
    ...
    if (!load_xref(pdf, xref_offset, password_cb, password_cbdata)) goto error;
  }
...
//
// 'pdfioFileOpenIO()' - Open a PDF using random-access input callbacks.
//
// This X4 extension keeps PDFio independent of SdFat while allowing the
// firmware to parse PDFs directly from the SD card.
//

pdfio_file_t* pdfioFileOpenIO(const char* name, pdfio_input_read_cb_t read_cb, pdfio_input_seek_cb_t seek_cb,
                              pdfio_input_close_cb_t close_cb, pdfio_xref_cb_t xref_cb, void* input_ctx,
                              pdfio_password_cb_t password_cb, void* password_cbdata, pdfio_error_cb_t error_cb,
                              void* error_cbdata) {
  pdfio_file_t* pdf;
  char line[1025], *ptr, *end;
  ssize_t bytes;
  off_t xref_offset;
  time_t curtime;
  ...
  if (!name || !read_cb || !seek_cb) return (NULL);
  ...
  if ((pdf = (pdfio_file_t*)calloc(1, sizeof(pdfio_file_t))) == NULL) {
    ...
    temp.filename = (char*)name;
    snprintf(message, sizeof(message), "Unable to allocate memory for PDF file: %s", strerror(errno));
    (error_cb)(&temp, message, error_cbdata);
    if (close_cb) (close_cb)(input_ctx);
    return (NULL);
  }

  pdf->loc = get_lconv();
  pdf->filename = strdup(name);
  pdf->mode = _PDFIO_MODE_READ;
  pdf->fd = -1;
  pdf->input_read_cb = read_cb;
  pdf->input_seek_cb = seek_cb;
  pdf->input_close_cb = close_cb;
  pdf->xref_cb = xref_cb;
  pdf->input_ctx = input_ctx;
  pdf->error_cb = error_cb;
  pdf->error_data = error_cbdata;
  pdf->permissions = PDFIO_PERMISSION_ALL;

  if (!pdf->filename) goto error;
  ...
  _pdfioCryptoSHA256Append(&ctx, (uint8_t*)pdf->filename, strlen(pdf->filename));
  _pdfioCryptoSHA256Append(&ctx, (uint8_t*)&curtime, sizeof(curtime));
  if (pdf->id_array && (id_value = pdfioArrayGetBinary(pdf->id_array, 0, &id_valuelen)) != NULL)
    _pdfioCryptoSHA256Append(&ctx, id_value, id_valuelen);
  if (pdf->id_array && (id_value = pdfioArrayGetBinary(pdf->id_array, 1, &id_valuelen)) != NULL)
    _pdfioCryptoSHA256Append(&ctx, id_value, id_valuelen);
  _pdfioCryptoSHA256Finish(&ctx, pdf->file_id);
...
void pdfioFileSetAuthor(pdfio_file_t* pdf,  // I - PDF file
...
void pdfioFileSetCreationDate(pdfio_file_t* pdf,  // I - PDF file
                              time_t value)       // I - Value
{
  if (pdf && pdf->info_obj) pdfioDictSetDate(pdf->info_obj->value.value.dict, "CreationDate", value);
}
...
void pdfioFileSetCreator(pdfio_file_t* pdf,  // I - PDF file
                         const char* value)  // I - Value
{
  if (pdf && pdf->info_obj)
...
void pdfioFileSetKeywords(pdfio_file_t* pdf,  // I - PDF file
                          const char* value)  // I - Value
{
  if (pdf && pdf->info_obj)
...
void pdfioFileSetLanguage(pdfio_file_t* pdf,  // I - PDF file
                          const char* value)  // I - Value
{
...
void pdfioFileSetModificationDate(pdfio_file_t* pdf,  // I - PDF file
                                  time_t value)       // I - Value
{
  if (pdf && pdf->info_obj) pdfioDictSetDate(pdf->info_obj->value.value.dict, "ModDate", value);
}
...
pdfioFileSetPermissions(pdfio_file_t* pdf,               // I - PDF file
                        ...
                        const char* user_password)       // I - User password, if any
{
  ...
  if (encryption == PDFIO_ENCRYPTION_NONE) return (true);
...
void pdfioFileSetSubject(pdfio_file_t* pdf,  // I - PDF file
                         const char* value)  // I - Value
{
  if (pdf && pdf->info_obj)
...
void pdfioFileSetTitle(pdfio_file_t* pdf,  // I - PDF file
                       const char* value)  // I - Value
{
  if (pdf && pdf->info_obj) pdfioDictSetString(pdf->info_obj->value.value.dict, "Title", pdfioStringCreate(pdf, value));
}
        ...
        off_t offset)               // I - Offset in file
{
  ...
  PDFIO_DEBUG("add_obj: obj=%p, ->pdf=%p, ->number=%lu, ->offset=%lu\n", (void*)obj, (void*)pdf,
              (unsigned long)obj->number, (unsigned long)offset);
...
//
// 'store_xref()' - Store a lazy cross-reference record.
//
// Cross-reference sections are visited newest first, so an existing record
// must never be replaced by an older incremental-update entry.
//

static bool store_xref(pdfio_file_t* pdf, size_t number, const pdfio_xref_t* xref) {
  pdfio_xref_t existing;

  if (!pdf->xref_cb || number < 1) return (true);

  memset(&existing, 0, sizeof(existing));
  if ((pdf->xref_cb)(pdf->input_ctx, number, &existing, false) && existing.type) return (true);

  if (!(pdf->xref_cb)(pdf->input_ctx, number, (pdfio_xref_t*)xref, true)) {
    _pdfioFileError(pdf, "Unable to store cross-reference for object %lu.", (unsigned long)number);
    return (false);
  }
...
create_common(const char* filename,         // I - Filename
              ...
              void* error_cbdata)           // I - Error callback data
{
  ...
  PDFIO_DEBUG(
      "create_common(filename=\"%s\", fd=%d, output_cb=%p, output_cbdata=%p, version=\"%s\", media_box=%p, "
      "crop_box=%p, error_cb=%p, error_cbdata=%p)\n",
      filename, fd, (void*)output_cb, (void*)output_cbdata, version, (void*)media_box, (void*)crop_box, (void*)error_cb,
      (void*)error_cbdata);

  // Range check input...
  if (!filename || (fd < 0 && !output_cb)) return (NULL);
  ...
  if ((dict = pdfioDictCreate(pdf)) == NULL) goto error;

  pdfioDictSetName(dict, "Type", "Pages");

  if ((pdf->pages_obj = pdfioFileCreateObj(pdf, dict)) == NULL) goto error;
  ...
  if ((pdf->info_obj = pdfioFileCreateObj(pdf, dict)) == NULL) goto error;
  ...
  if ((pdf->root_obj = pdfioFileCreateObj(pdf, dict)) == NULL) goto error;
             ...
             size_t bufsize)  // I - Size of date buffer
{
  ...
  snprintf(buffer, bufsize, "%04d-%02d-%02dT%02d:%02d:%02dZ", d.tm_year + 1900, d.tm_mon + 1, d.tm_mday, d.tm_hour,
           d.tm_min, d.tm_sec);
...
get_lconv(void) {
  ...
  if ((loc = localeconv()) != NULL) {
    ...
    if (!loc->decimal_point || !strcmp(loc->decimal_point, ".")) loc = NULL;
  }
...
//
// 'get_lazy_page()' - Resolve one page through the page tree on demand.
//
// A conventional leaf /Pages node has Count == Kids length.  In that common
// case we can jump directly to the requested child without loading every page
// dictionary before it.  Higher tree levels only load their small /Pages
// nodes to use the subtree Count values.
//

static pdfio_obj_t* get_lazy_page(pdfio_file_t* pdf, pdfio_obj_t* obj, size_t page, size_t depth) {
  pdfio_dict_t *dict, *child_dict;
  pdfio_array_t* kids;
  pdfio_obj_t* child;
  const char *type, *child_type;
  size_t i, num_kids, count;

  PDFIO_DEBUG("get_lazy_page: obj=%lu page=%lu depth=%lu\n", obj ? (unsigned long)obj->number : 0, (unsigned long)page,
              (unsigned long)depth);
  if (!obj || depth >= PDFIO_MAX_DEPTH || (dict = pdfioObjGetDict(obj)) == NULL) return (NULL);

  type = pdfioDictGetName(dict, "Type");
  if (type && !strcmp(type, "Page")) return (page == 0 ? obj : NULL);

  if ((kids = pdfioDictGetArray(dict, "Kids")) == NULL || (num_kids = pdfioArrayGetSize(kids)) == 0) return (NULL);

  count = (size_t)pdfioDictGetNumber(dict, "Count");
  PDFIO_DEBUG("get_lazy_page: obj=%lu count=%lu kids=%lu\n", (unsigned long)obj->number, (unsigned long)count,
              (unsigned long)num_kids);
  if (count == num_kids && page < num_kids) {
    // Standard leaf page-tree node: resolve just the desired page.
    child = pdfioArrayGetObj(kids, page);
    if (!child || (child_dict = pdfioObjGetDict(child)) == NULL) return (NULL);
    child_type = pdfioDictGetName(child_dict, "Type");
    if (!child_type || strcmp(child_type, "Pages")) return (child);
    return (get_lazy_page(pdf, child, 0, depth + 1));
  }

  for (i = 0; i < num_kids; i++) {
    if ((child = pdfioArrayGetObj(kids, i)) == NULL || (child_dict = pdfioObjGetDict(child)) == NULL) return (NULL);

    child_type = pdfioDictGetName(child_dict, "Type");
    if (child_type && !strcmp(child_type, "Page")) {
      if (page == 0) return (child);
      page--;
    } else {
      count = (size_t)pdfioDictGetNumber(child_dict, "Count");
      if (page < count) return (get_lazy_page(pdf, child, page, depth + 1));
      page -= count;
    }
  }
...
load_obj_stream(pdfio_obj_t* obj)  // I - Object to load
{
  ...
  pdfio_obj_t* objs[1024];  // Objects (bounded for ESP32-C3)
  ...
  while (count > 0 && _pdfioTokenGet(&tb, buffer, sizeof(buffer))) {
    ...
    if ((objs[num_objs] = pdfioFileFindObj(obj->pdf, number)) == NULL) objs[num_objs] = add_obj(obj->pdf, number, 0, 0);
...
//
// '_pdfioFileLoadCompressedObj()' - Load one value from an object stream.
//
// The normal desktop reader expands every value in an object stream.  The X4
// callback reader instead reopens and decodes the containing stream for the
// one requested value, trading a little SD I/O for a large RAM saving.
//

bool _pdfioFileLoadCompressedObj(pdfio_obj_t* obj) {
  pdfio_obj_t* stream_obj;
  pdfio_dict_t* dict;
  pdfio_stream_t* st;
  _pdfio_token_t tb;
  char buffer[32];
  size_t count, first, i, number, value_offset = 0;
  bool found = false;

  if (!obj || !obj->object_stream || (stream_obj = pdfioFileFindObj(obj->pdf, obj->object_stream)) == NULL ||
      (dict = pdfioObjGetDict(stream_obj)) == NULL) {
    if (obj) _pdfioFileError(obj->pdf, "Unable to find object stream for object %lu.", (unsigned long)obj->number);
    return (false);
  }

  count = (size_t)pdfioDictGetNumber(dict, "N");
  first = (size_t)pdfioDictGetNumber(dict, "First");
  if (!count || !first || (st = pdfioObjOpenStream(stream_obj, true)) == NULL) {
    _pdfioFileError(obj->pdf, "Unable to open object stream %lu.", (unsigned long)obj->object_stream);
    return (false);
  }

  _pdfioTokenInit(&tb, obj->pdf, (_pdfio_tconsume_cb_t)pdfioStreamConsume, (_pdfio_tpeek_cb_t)pdfioStreamPeek, st);
  for (i = 0; i < count; i++) {
    if (!_pdfioTokenGet(&tb, buffer, sizeof(buffer)) || !isdigit(buffer[0] & 255)) break;
    number = (size_t)strtoimax(buffer, NULL, 10);
    if (!_pdfioTokenGet(&tb, buffer, sizeof(buffer)) || !isdigit(buffer[0] & 255)) break;
    if (number == obj->number) {
      value_offset = (size_t)strtoimax(buffer, NULL, 10);
      found = true;
      break;
    }
  }
  ...
  if (!found || (st = pdfioObjOpenStream(stream_obj, true)) == NULL) {
    _pdfioFileError(obj->pdf, "Unable to locate compressed object %lu.", (unsigned long)obj->number);
    return (false);
  }

  if (!pdfioStreamConsume(st, first + value_offset)) {
    _pdfioFileError(obj->pdf, "Unable to seek to compressed object %lu.", (unsigned long)obj->number);
    pdfioStreamClose(st);
    return (false);
  }
  ...
  if (!_pdfioValueRead(obj->pdf, obj, &tb, &obj->value, 0)) {
    _pdfioFileError(obj->pdf, "Unable to read compressed object %lu.", (unsigned long)obj->number);
    pdfioStreamClose(st);
    return (false);
  }
           ...
           size_t depth)       // I - Depth of page tree
{
  ...
  if ((type = pdfioDictGetName(dict, "Type")) == NULL || (strcmp(type, "Pages") && strcmp(type, "Page"))) {
    if (!_pdfioFileError(pdf, "WARNING: No Type value for pages object.")) return (false);
  }
  ...
  if ((kids = pdfioDictGetArray(dict, "Kids")) != NULL) {
    ...
    for (i = 0, num_kids = pdfioArrayGetSize(kids); i < num_kids; i++) {
      if (!load_pages(pdf, pdfioArrayGetObj(kids, i), depth + 1)) return (false);
    }
  } else {
    // Add this page...
    if (pdf->num_pages >= pdf->alloc_pages) {
          ...
          void* password_data)              // I - Password callback data, if any
{
  ...
  while (!done) {
    ...
    if (isdigit(line[0] & 255) && strlen(line) > 4 &&
        (!strcmp(line + strlen(line) - 4, " obj") ||
         ((ptr = strstr(line, " obj")) != NULL && (ptr[4] == '<' || isspace(ptr[4]))))) {
      ...
      size_t num_sobjs = 0,        // Number of object streams
          sobjs[256];              // Object streams to load
      ...
      PDFIO_DEBUG("load_xref: tb.bufptr=%p, tb.bufend=%p, tb.bufptr[0]=0x%02x, tb.bufptr[0]=0x%02x\n", (void*)tb.bufptr,
                  (void*)tb.bufend, tb.bufptr[0], tb.bufptr[1]);
      ...
      if (pdfioArrayGetSize(w_array) > 3 || w[1] == 0 || w[2] > 4 || w[0] > sizeof(buffer) || w[1] > sizeof(buffer) ||
          w[2] > sizeof(buffer) || w_total > sizeof(buffer)) {
      ...
      for (index_n = 0; index_n < index_count; index_n += 2) {
        ...
        while (count > 0 && pdfioStreamRead(st, buffer, w_total) > 0) {
          ...
          // Offset
          for (i = 1, offset = buffer[w_2]; i < w[1]; i++) offset = (offset << 8) | buffer[w_2 + i];
          ...
          switch (w[2]) {
            ...
            case 3:
              ...
              if (generation > 65535) generation = 65535;
              break;
          ...
          if (pdf->xref_cb) {
            // Keep the complete table on the SD card.  Type 2 records name the
            // containing object stream; the individual value is decoded lazily.
            pdfio_xref_t xref;

            memset(&xref, 0, sizeof(xref));
            if (w[0] > 0 && buffer[0] == 2) {
              xref.type = 2;
              xref.object_stream = (size_t)offset;
            } else {
              xref.type = 1;
              xref.offset = (off_t)offset;
              xref.generation = (unsigned short)generation;
            }

            if (!store_xref(pdf, (size_t)number, &xref)) {
              pdfioStreamClose(st);
              return (false);
            }
          } else {
            // Desktop/default mode keeps the original eager object table.
            ...
            if (w[0] > 0 && buffer[0] == 2) {
              for (i = 0; i < num_sobjs; i++)
                if (sobjs[i] == (size_t)offset) break;
            ...
            } else if (!current && !add_obj(pdf, (size_t)number, (unsigned short)generation, (off_t)offset)) {
              pdfioStreamClose(st);
              return (false);
            }
          }
      ...
      for (i = 0; !pdf->xref_cb && i < num_sobjs; i++) {
        if ((obj = pdfioFileFindObj(pdf, sobjs[i])) != NULL) {
          ...
          if (!load_obj_stream(obj)) return (false);
        } else {
      ...
      while (_pdfioFileGets(pdf, line, sizeof(line), false)) {
        ...
        for (; num_objects > 0; num_objects--, number++) {
          ...
          if (*ptr == 'f') continue;  // Don't care about free objects...
          ...
          if (pdf->xref_cb) {
            ...
            xref.offset = (off_t)offset;
            xref.generation = (unsigned short)generation;
            if (!store_xref(pdf, (size_t)number, &xref)) return (false);
          } else {
            // Create a placeholder for the object in memory...
            if (pdfioFileFindObj(pdf, (size_t)number)) continue;  // Don't replace newer object...

            if (!add_obj(pdf, (size_t)number, (unsigned short)generation, (off_t)offset)) return (false);
          }
        }
  ...
  if (pdf->xref_cb) {
    pdfio_dict_t* pages_dict = pdfioObjGetDict(pages_obj);

    // Callback readers resolve pages lazily so a 500-page book does not keep
    // 500 page/resource dictionaries resident before conversion even starts.
    if (!pages_dict || (pdf->num_pages = (size_t)pdfioDictGetNumber(pages_dict, "Count")) == 0) {
      _pdfioFileError(pdf, "Unable to determine PDF page count.");
      return (false);
    }
    pdf->pages_obj = pages_obj;
    return (true);
  }
...
static bool                                   // O - `true` on success, `false` on failure
repair_xref(pdfio_file_t* pdf,                // I - PDF file
            pdfio_password_cb_t password_cb,  // I - Password callback or `NULL` for none
            void* password_data)              // I - Password callback data, if any
{
  char line[1024],                      // Line from file
  ...
  size_t i;                             // Looping var
  size_t num_sobjs = 0;                 // Number of object streams
  pdfio_obj_t* sobjs[256];              // Object streams to load
  ...
  if ((line_offset = _pdfioFileSeek(pdf, 0, SEEK_SET)) < 0) return (false);

  while (_pdfioFileGets(pdf, line, sizeof(line), true)) {
    // See if this is the start of an object...
    if (line[0] >= '1' && line[0] <= '9') {
      // Maybe, look some more...
      if ((number = strtoimax(line, &ptr, 10)) >= 1 && (generation = (int)strtol(ptr, &ptr, 10)) >= 0 &&
          generation < 65536) {
        while (isspace(*ptr & 255)) ptr++;
          ...
          if (_pdfioTokenGet(&tb, line, sizeof(line))) {
            ...
            if (!strcmp(line, "stream")) {
              ...
              if (!strcmp(type, "Catalog")) {
                ...
                if (!backup_trailer) backup_trailer = pdfioDictCreate(pdf);
  ...
  if (!pdf->trailer_dict && backup_trailer) pdf->trailer_dict = backup_trailer;
  ...
  for (i = 0; i < num_sobjs; i++) {
    if (!load_obj_stream(sobjs[i])) return (false);
  }
...
write_metadata(pdfio_file_t* pdf)  // I - PDF file
{
  ...
  if ((dict = pdfioDictCreate(pdf)) == NULL) return (false);

  pdfioDictSetName(dict, "Type", "Metadata");
  pdfioDictSetName(dict, "Subtype", "XML");

  if ((obj = pdfioFileCreateObj(pdf, dict)) == NULL) return (false);

  // Write the XMP stream...
  if ((st = pdfioObjCreateStream(obj, PDFIO_FILTER_NONE)) == NULL) return (false);
  ...
  status &=
      pdfioStreamPuts(st, "        <rdf:Description rdf:about=\"\" xmlns:xmp=\"http://ns.adobe.com/xap/1.0/\">\n");
  ...
  status &=
      pdfioStreamPuts(st, "        <rdf:Description rdf:about=\"\" xmlns:pdf=\"http://ns.adobe.com/pdf/1.3/\">\n");
  ...
  status &=
      pdfioStreamPuts(st, "        <rdf:Description rdf:about=\"\" xmlns:dc=\"http://purl.org/dc/elements/1.1/\">\n");
  status &= pdfioStreamPrintf(st, "            <dc:format>application/pdf</dc:format>\n");
  if ((value = pdfioFileGetTitle(pdf)) != NULL)
    status &= pdfioStreamPrintf(
        st, "            <dc:title><rdf:Alt><rdf:li xml:lang=\"x-default\">%H</rdf:li></rdf:Alt></dc:title>\n", value);
  if ((value = pdfioFileGetAuthor(pdf)) != NULL)
    status &=
        pdfioStreamPrintf(st, "            <dc:creator><rdf:Seq><rdf:li>%H</rdf:li></rdf:Seq></dc:creator>\n", value);
  if ((value = pdfioFileGetSubject(pdf)) != NULL)
    status &= pdfioStreamPrintf(
        st,
        "            <dc:description><rdf:Alt><rdf:li xml:lang=\"x-default\">%H</rdf:li></rdf:Alt></dc:description>\n",
        value);
  status &= pdfioStreamPuts(st, "        </rdf:Description>\n");
  ...
  if (pdf->profile >= _PDFIO_PROFILE_PDFA_1A && pdf->profile <= _PDFIO_PROFILE_PDFA_4) {
    ...
    status &=
        pdfioStreamPuts(st, "     <rdf:Description rdf:about=\"\" xmlns:pdfaid=\"http://www.aiim.org/pdfa/ns/id/\">\n");
    status &= pdfioStreamPrintf(st, "       <pdfaid:part>%c</pdfaid:part>\n", *info);
    if (info[1]) status &= pdfioStreamPrintf(st, "       <pdfaid:conformance>%s</pdfaid:conformance>\n", info + 1);
    status &= pdfioStreamPuts(st, " </rdf:Description>\n");
  }
  ...
  if (!pdfioDictSetObj(pdfioFileGetCatalog(pdf), "Metadata", obj)) return (false);
...
write_pages(pdfio_file_t* pdf)  // I - PDF file
{
  ...
  if ((kids = pdfioArrayCreate(pdf)) == NULL) return (false);

  for (i = 0; i < pdf->num_pages; i++) pdfioArrayAppendObj(kids, pdf->pages[i]);
...
write_trailer(pdfio_file_t* pdf)  // I - PDF file
{
  ...
  if (strcmp(pdf->version, "1.5") >= 0 && !pdf->output_cb) {
    ...
    if (pdf->encrypt_obj) pdfioDictSetObj(xref_dict, "Encrypt", pdf->encrypt_obj);

    if (pdf->id_array) pdfioDictSetArray(xref_dict, "ID", pdf->id_array);
    ...
    if (pdf->encrypt_obj) pdfioDictSetObj(pdf->trailer_dict, "Encrypt", pdf->encrypt_obj);
    if (pdf->id_array) pdfioDictSetArray(pdf->trailer_dict, "ID", pdf->id_array);
