    ...
    size_t       number)		// I - Object number (1 to N)
{
  ...
  if ((obj = find_loaded_obj(pdf, number)) != NULL)
    return (obj);
  ...
  if (!(pdf->xref_cb)(pdf->input_ctx, number, &xref, false) || !xref.type)
    return (NULL);
  ...
  {
    if ((obj = add_obj(pdf, number, 0, 0)) != NULL)
      obj->object_stream = xref.object_stream;
    return (obj);
  }
...
pdfioFileOpenIO(
    const char             *name,
    pdfio_input_read_cb_t  read_cb,
    pdfio_input_seek_cb_t  seek_cb,
    pdfio_input_close_cb_t close_cb,
    pdfio_xref_cb_t        xref_cb,
    void                   *input_ctx,
...
store_xref(
    pdfio_file_t       *pdf,
    size_t             number,
    const pdfio_xref_t *xref)
{
  pdfio_xref_t existing;


  if (!pdf->xref_cb || number < 1)
    return (true);

  memset(&existing, 0, sizeof(existing));
  if ((pdf->xref_cb)(pdf->input_ctx, number, &existing, false) && existing.type)
    return (true);
    ...
    size_t       depth)
{
  ...
  PDFIO_DEBUG("get_lazy_page: obj=%lu page=%lu depth=%lu\n", obj ? (unsigned long)obj->number : 0,
              (unsigned long)page, (unsigned long)depth);
  if (!obj || depth >= PDFIO_MAX_DEPTH || (dict = pdfioObjGetDict(obj)) == NULL)
    return (NULL);

  type = pdfioDictGetName(dict, "Type");
  if (type && !strcmp(type, "Page"))
    return (page == 0 ? obj : NULL);

  if ((kids = pdfioDictGetArray(dict, "Kids")) == NULL ||
      (num_kids = pdfioArrayGetSize(kids)) == 0)
    return (NULL);

  count = (size_t)pdfioDictGetNumber(dict, "Count");
  PDFIO_DEBUG("get_lazy_page: obj=%lu count=%lu kids=%lu\n", (unsigned long)obj->number,
              (unsigned long)count, (unsigned long)num_kids);
  if (count == num_kids && page < num_kids)
  {
    ...
    if (!child || (child_dict = pdfioObjGetDict(child)) == NULL)
      return (NULL);
    child_type = pdfioDictGetName(child_dict, "Type");
    if (!child_type || strcmp(child_type, "Pages"))
      return (child);
  ...
  {
    if ((child = pdfioArrayGetObj(kids, i)) == NULL ||
        (child_dict = pdfioObjGetDict(child)) == NULL)
      return (NULL);
    ...
    else
    {
      ...
      if (page < count)
        return (get_lazy_page(pdf, child, page, depth + 1));
      page -= count;
    }
  }
...
bool
_pdfioFileLoadCompressedObj(pdfio_obj_t *obj)
{
  ...
  if (!obj || !obj->object_stream ||
      (stream_obj = pdfioFileFindObj(obj->pdf, obj->object_stream)) == NULL ||
      (dict = pdfioObjGetDict(stream_obj)) == NULL)
  {
    if (obj)
      _pdfioFileError(obj->pdf, "Unable to find object stream for object %lu.", (unsigned long)obj->number);
    return (false);
  }
  ...
  _pdfioTokenInit(&tb, obj->pdf, (_pdfio_tconsume_cb_t)pdfioStreamConsume,
                  (_pdfio_tpeek_cb_t)pdfioStreamPeek, st);
  for (i = 0; i < count; i ++)
  {
    if (!_pdfioTokenGet(&tb, buffer, sizeof(buffer)) || !isdigit(buffer[0] & 255))
      break;
    number = (size_t)strtoimax(buffer, NULL, 10);
    if (!_pdfioTokenGet(&tb, buffer, sizeof(buffer)) || !isdigit(buffer[0] & 255))
      break;
