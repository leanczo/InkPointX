...
static void md5_process(_pdfio_md5_t* pms,             // I - MD5 state
...
void _pdfioCryptoMD5Init(_pdfio_md5_t* pms)  // I - MD5 state
{
...
void _pdfioCryptoMD5Append(_pdfio_md5_t* pms,    // I - MD5 state
...
void _pdfioCryptoMD5Finish(_pdfio_md5_t* pms,   // I - MD5 state
                           uint8_t digest[16])  // O - Digest value
{
  ...
  for (i = 0; i < 16; ++i) digest[i] = (uint8_t)(pms->abcd[i >> 2] >> ((i & 3) << 3));
}
