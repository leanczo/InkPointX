...
void _pdfioCryptoAESInit(_pdfio_aes_t* ctx,   // I - AES context
                         ...
                         const uint8_t* iv)   // I - 16-byte initialization vector
{
  ...
  for (rkptr0 = ctx->round_key, rkptr = rkptr0 + keylen, rkend = rkptr + 16 * ctx->round_size, i = nwords;
       rkptr < rkend; i++) {
    if ((i % nwords) == 0) {
  ...
  if (iv) memcpy(ctx->iv, iv, sizeof(ctx->iv));
}
...
_pdfioCryptoAESDecrypt(_pdfio_aes_t* ctx,        // I - AES context
                       ...
                       bool last)                // I - Is this the last block?
{
  ...
  if (last) {
    if (outbuffer[-1] <= 0x10) outbytes -= outbuffer[-1];
  }
...
size_t                                           // O - Number of bytes in output buffer
_pdfioCryptoAESEncrypt(_pdfio_aes_t* ctx,        // I - AES context
                       uint8_t* outbuffer,       // I - Output buffer
                       const uint8_t* inbuffer,  // I - Input buffer
...
static void add_round_key(size_t round,              // I - Which round
                          ...
                          const uint8_t* round_key)  // I - Key
{
  ...
  for (round_key += round * 16, i = 16; i > 0; i--, sptr++, round_key++) *sptr ^= *round_key;
}
...
static void sub_bytes(state_t* state)  // I - Current state
{
  unsigned i;                   // Looping var
  uint8_t* sptr = (*state)[0];  // Pointer into state

  for (i = 16; i > 0; i--, sptr++) *sptr = sbox[*sptr];
}
...
static void shift_rows(state_t* state)  // I - Current state
{
  uint8_t* sptr = (*state)[0];  // Pointer into state
...
static void mix_columns(state_t* state)  // I - Current state
{
  unsigned i;                   // Looping var
  uint8_t* sptr = (*state)[0];  // Pointer into state
...
static void inv_mix_columns(state_t* state)  // I - Current state
{
  unsigned i;                   // Looping var
  uint8_t* sptr = (*state)[0];  // Pointer into state
...
static void inv_sub_bytes(state_t* state)  // I - Current state
{
  unsigned i;                   // Looping var
  uint8_t* sptr = (*state)[0];  // Pointer into state

  for (i = 16; i > 0; i--, sptr++) *sptr = rsbox[*sptr];
}
...
static void inv_shift_rows(state_t* state)  // I - Current state
{
  uint8_t* sptr = (*state)[0];  // Pointer into state
  uint8_t temp;                 // Temporary value
...
static void cipher(state_t* state,           // I - Current state
...
static void inv_cipher(state_t* state,           // I - Current state
                       const _pdfio_aes_t* ctx)  // I - AES context
{
...
static void xor_with_iv(uint8_t* buf,       // I - Block
