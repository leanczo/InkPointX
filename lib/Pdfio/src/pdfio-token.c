...
void _pdfioTokenClear(_pdfio_token_t* tb)  // I - Token buffer/stack
{
...
void _pdfioTokenFlush(_pdfio_token_t* tb)  // I - Token buffer/stack
{
  ...
  if (tb->num_tokens > 0) {
    ...
    if ((len = strlen(tb->tokens[tb->num_tokens])) > (bufsize - 1)) {
      ...
      PDFIO_DEBUG("_pdfioTokenGet(tb=%p, buffer=%p, bufsize=%u): Token '%s' from stack too large.\n", (void*)tb,
                  (void*)buffer, (unsigned)bufsize, tb->tokens[tb->num_tokens]);
      *buffer = '\0';
      return (false);
    }
...
void _pdfioTokenInit(_pdfio_token_t* ts,               // I - Token buffer/stack
...
void _pdfioTokenPush(_pdfio_token_t* tb,  // I - Token buffer/stack
                     const char* token)   // I - Token to push
{
  if (tb->num_tokens < (sizeof(tb->tokens) / sizeof(tb->tokens[0]))) {
    if ((tb->tokens[tb->num_tokens++] = strdup(token)) == NULL) tb->num_tokens--;
  }
}
                ...
                size_t bufsize)      // I - Size of string buffer
{
  ...
  switch (state) {
    case '(':  // Literal string
      ...
      if ((bufptr - buffer) >= 3 && ((bufptr - buffer) & 1) != 0 &&
          (!memcmp(buffer, "(\377\376", 3) || !memcmp(buffer, "(\376\377", 3))) {
