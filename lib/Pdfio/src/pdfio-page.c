              ...
              pdfio_obj_t* srcpage)  // I - Source page
{
  ...
  PDFIO_DEBUG("pdfioPageCopy(pdf=%p, srcpage=%p(%p))\n", (void*)pdf, (void*)srcpage,
              srcpage ? (void*)srcpage->pdf : NULL);
  ...
  if ((dstdict = pdfioDictCreate(pdf)) == NULL) return (false);
  ...
  do {
    ...
    if (parent->value.type != PDFIO_VALTYPE_DICT) break;
    ...
    for (i = srcdict->num_pairs, srcpair = srcdict->pairs; i > 0; i--, srcpair++) {
      ...
      if (!strcmp(srcpair->key, "Count") || !strcmp(srcpair->key, "Kids")) {
        ...
        // New key/value pair...
        _pdfioDictSetValue(dstdict, pdfioStringCreate(pdf, srcpair->key),
                           _pdfioValueCopy(pdf, &dstvalue, srcpage->pdf, &srcpair->value));
      }
    }
  } while (parent != NULL && depth < PDFIO_MAX_DEPTH);

  // Make sure the page dictionary has all of the required keys...
  if (!_pdfioDictGetValue(dstdict, "MediaBox")) pdfioDictSetRect(dstdict, "MediaBox", &srcpage->pdf->media_box);

  pdfioDictSetObj(dstdict, "Parent", pdf->pages_obj);

  if (!_pdfioDictGetValue(dstdict, "Type")) pdfioDictSetName(dstdict, "Type", "Page");

  // Create the page object and add it to the pages array...
  if ((dstpage = pdfioFileCreateObj(pdf, dstdict)) == NULL) return (false);
...
pdfioPageGetNumStreams(pdfio_obj_t* page)  // I - Page object
{
...
pdfioPageOpenStream(pdfio_obj_t* page,  // I - Page object
...
get_contents(pdfio_obj_t* page)  // I - Page object
{
  ...
  if (page->value.type == PDFIO_VALTYPE_NONE) {
    if (!_pdfioObjLoad(page)) return (NULL);
  }

  if (page->value.type != PDFIO_VALTYPE_DICT) return (NULL);

  if ((contents = _pdfioDictGetValue(page->value.value.dict, "Contents")) == NULL) return (NULL);
  ...
  if (contents->type == PDFIO_VALTYPE_INDIRECT) {
    ...
    if ((obj = pdfioFileFindObj(page->pdf, contents->value.indirect.number)) != NULL) {
      ...
      if (obj->value.type == PDFIO_VALTYPE_ARRAY) contents = &(obj->value);
    }
  }
