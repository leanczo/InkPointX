...
_pdfio_value_t* _pdfioValueCopy(pdfio_file_t* pdfdst,  // I - Destination PDF file
                                ...
                                _pdfio_value_t* vsrc)  // I - Source value
{
  ...
  static const char* const types[] =  // Type strings for debug
      {"PDFIO_VALTYPE_NONE", "PDFIO_VALTYPE_ARRAY",  "PDFIO_VALTYPE_BINARY",   "PDFIO_VALTYPE_BOOLEAN",
       "PDFIO_VALTYPE_DATE", "PDFIO_VALTYPE_DICT",   "PDFIO_VALTYPE_INDIRECT", "PDFIO_VALTYPE_NAME",
       "PDFIO_VALTYPE_NULL", "PDFIO_VALTYPE_NUMBER", "PDFIO_VALTYPE_STRING"};
#endif  // DEBUG

  PDFIO_DEBUG("_pdfioValueCopy(pdfdst=%p, vdst=%p, pdfsrc=%p, vsrc=%p(%s))\n", (void*)pdfdst, (void*)vdst,
              (void*)pdfsrc, (void*)vsrc, types[vsrc->type]);
  ...
  switch (vsrc->type) {
    ...
    case PDFIO_VALTYPE_ARRAY:
      if ((vdst->value.array = pdfioArrayCopy(pdfdst, vsrc->value.array)) == NULL) return (NULL);
      break;
    ...
    case PDFIO_VALTYPE_DICT:
      if ((vdst->value.dict = pdfioDictCopy(pdfdst, vsrc->value.dict)) == NULL) return (NULL);
      break;

    case PDFIO_VALTYPE_NAME:
    case PDFIO_VALTYPE_STRING:
      if ((vdst->value.name = pdfioStringCreate(pdfdst, vsrc->value.name)) == NULL) return (NULL);
      break;
  }
                   ...
                   size_t depth)       // I - Depth
{
  ...
  switch (v->type) {
    ...
    case PDFIO_VALTYPE_BINARY:
      ...
      if ((cb = _pdfioCryptoMakeReader(pdf, obj, &ctx, v->value.binary.data, &ivlen)) == NULL) return (false);
    ...
    case PDFIO_VALTYPE_STRING:
      ...
      ivlen = templen;
      if ((cb = _pdfioCryptoMakeReader(pdf, obj, &ctx, (uint8_t*)v->value.string, &ivlen)) == NULL) return (false);
...
void _pdfioValueDebug(_pdfio_value_t* v,  // I - Value
                      FILE* fp)           // I - Output file
{
  ...
  switch (v->type) {
    ...
    case PDFIO_VALTYPE_BINARY: {
      ...
      putc('<', fp);
      for (i = v->value.binary.datalen, ptr = v->value.binary.data; i > 0; i--, ptr++) fprintf(fp, "%02X", *ptr);
      putc('>', fp);
    } break;
    case PDFIO_VALTYPE_BOOLEAN:
    ...
    case PDFIO_VALTYPE_DATE: {
      ...
      fprintf(fp, "(D:%04d%02d%02d%02d%02d%02dZ)", dateval.tm_year + 1900, dateval.tm_mon + 1, dateval.tm_mday,
              dateval.tm_hour, dateval.tm_min, dateval.tm_sec);
    } break;
    case PDFIO_VALTYPE_DICT:
      fputs("<<", fp);
...
void _pdfioValueDelete(_pdfio_value_t* v)  // I - Value
{
  if (v->type == PDFIO_VALTYPE_BINARY) free(v->value.binary.data);
}
                ...
                size_t depth)        // I - Depth of value
{
  ...
  if (!token) {
    _pdfioFileError(pdf, "Unable to allocate PDF value token buffer.");
    goto done;
  }

  if (!_pdfioTokenGet(tb, token, PDFIO_MAX_STRING)) goto done;
  ...
  if (!strcmp(token, "[")) {
    ...
    if ((v->value.array = _pdfioArrayRead(pdf, obj, tb, depth + 1)) == NULL) goto done;
    ...
    if ((v->value.dict = _pdfioDictRead(pdf, obj, tb, depth + 1)) == NULL) goto done;
    ...
    if (isdigit((unsigned char)token[0]) && !strchr(token, '.')) {
      ...
      if ((tb->bufend - tb->bufptr) < 10) {
        ...
        if ((bytes = (tb->peek_cb)(tb->cb_data, tb->buffer, sizeof(tb->buffer))) > 0) tb->bufend = tb->buffer + bytes;
      ...
      while (tempptr < tb->bufend && isspace(*tempptr & 255)) tempptr++;  // Skip whitespace as needed...
      ...
      if (tempptr < tb->bufend && isdigit(*tempptr & 255)) {
        ...
        while (tempptr < tb->bufend && isspace(*tempptr & 255)) tempptr++;  // Skip whitespace
  ...
  if (token) _pdfioStringFreeBuffer(pdf, token);
                 ...
                 off_t* length)      // O - Offset to /Length value, if any
{
  switch (v->type) {
    ...
    case PDFIO_VALTYPE_BINARY: {
      ...
      if (!_pdfioFilePuts(pdf, "<")) goto bindone;

      for (; databytes > 1; databytes -= 2, dataptr += 2) {
        if (!_pdfioFilePrintf(pdf, "%02X%02X", dataptr[0], dataptr[1])) goto bindone;
      }

      if (databytes > 0 && !_pdfioFilePrintf(pdf, "%02X", dataptr[0])) goto bindone;

      ret = _pdfioFilePuts(pdf, ">");

    bindone:

      if (temp) _pdfioStringFreeBuffer(pdf, (char*)temp);
    ...
    case PDFIO_VALTYPE_DATE: {
      ...
      snprintf(datestr, sizeof(datestr), "D:%04d%02d%02d%02d%02d%02dZ", date.tm_year + 1900, date.tm_mon + 1,
      ...
      if (obj && pdf->encryption) {
        ...
        if (!_pdfioFilePuts(pdf, "<")) return (false);

        for (tempptr = temp; tempbytes > 1; tempbytes -= 2, tempptr += 2) {
          if (!_pdfioFilePrintf(pdf, "%02X%02X", tempptr[0], tempptr[1])) return (false);
        }
    ...
    case PDFIO_VALTYPE_INDIRECT:
      return (
          _pdfioFilePrintf(pdf, " %lu %u R", (unsigned long)v->value.indirect.number, v->value.indirect.generation));
    ...
    case PDFIO_VALTYPE_STRING:
      if (obj && pdf->encryption) {
        ...
        if (!_pdfioFilePuts(pdf, "<")) goto strdone;

        for (tempptr = temp; tempbytes > 1; tempbytes -= 2, tempptr += 2) {
          if (!_pdfioFilePrintf(pdf, "%02X%02X", tempptr[0], tempptr[1])) goto strdone;
        }

        if (tempbytes > 0 && !_pdfioFilePrintf(pdf, "%02X", *tempptr)) goto strdone;
...
get_date_time(const char* s)  // I - PDF date/time value
{
  ...
  if (strncmp(s, "D:", 2)) return (0);

  for (i = 2; i < 16; i++) {
    // Look for date/time digits...
    if (!isdigit(s[i] & 255) || !s[i]) break;
  }
  ...
  if (i > 6) dateval.tm_mon = (s[6] - '0') * 10 + s[7] - '0' - 1;
  if (i > 8)
    dateval.tm_mday = (s[8] - '0') * 10 + s[9] - '0';
  else
    dateval.tm_mday = 1;
  if (i > 10) dateval.tm_hour = (s[10] - '0') * 10 + s[11] - '0';
  if (i > 12) dateval.tm_min = (s[12] - '0') * 10 + s[13] - '0';
  if (i > 14) dateval.tm_sec = (s[14] - '0') * 10 + s[15] - '0';
  ...
  if (i >= 16 && s[i]) {
    ...
    if (s[i] == 'Z') {
      ...
      if (isdigit(s[i + 1] & 255) && isdigit(s[i + 2] & 255) && s[i + 3] == '\'' && isdigit(s[i + 4] & 255) &&
          isdigit(s[i + 5] & 255)) {
...
#if _WIN32
  if ((t = _mkgmtime(&dateval)) <= 0) return (0);

#elif defined(HAVE_TIMEGM)
  if ((t = timegm(&dateval)) <= 0) return (0);

#else
  if ((t = mktime(&dateval)) <= 0) return (0);
